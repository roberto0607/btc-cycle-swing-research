...
### 2.1 Primary dataset
- **Sources:** Kraken public REST (`GET /0/public/OHLC`, no auth, pair
  `XBTUSD`, interval `1440` = daily) and Coinbase Exchange public candles
  endpoint (no auth, product `BTC-USD`, daily granularity).
- **Coverage:** 2014-01-01 → present, daily OHLCV.
- **Rationale:** both free, no-auth, US-accessible (satisfies the project's
  data-source rule — no geoblocking risk), both liquid USD spot venues with
  clean daily history from 2014 on. Using two sources lets Phase 2+
  research distinguish real market behavior from single-exchange artifacts
  (master spec Section 50).
- **Role of each source:** Kraken is primary/reference series for all
  strategy and backtest logic. Coinbase is the cross-validation series —
  every major statistical finding gets a "does this hold on Coinbase too"
